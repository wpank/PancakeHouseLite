...
PancakeHouseLite::PancakeHouseLite(IPlugInstanceInfo instanceInfo)
	...
	filterEnvelopeAmount(0.0)
{
  ...
  mMIDIReceiver.noteOn.Connect(this, &PancakeHouseLite::onNoteOn);
  mMIDIReceiver.noteOff.Connect(this, &PancakeHouseLite::onNoteOff);

  mEnvelopeGenerator.beganEnvelopeCycle.Connect(this, &PancakeHouseLite::onBeganEvelopeCycle);
  mEnvelopeGenerator.finishedEnvelopeCycle.Connect(this, &PancakeHouseLite::onFinishedEnvelopeCycle);
}

PancakeHouseLite::~PancakeHouseLite() {}

void PancakeHouseLite::ProcessDoubleReplacing(double** inputs, 
	double** outputs, 
	int nFrames)
{
...
void PancakeHouseLite::OnParamChange(int paramIdx)
{
  IMutexLock lock(this);
  switch (paramIdx) {
	case mWaveform:
...
void PancakeHouseLite::CreatePresets() {
	
}

void PancakeHouseLite::ProcessMidiMsg(IMidiMsg* pMsg) {
	mMIDIReceiver.onMessageReceived(pMsg);
	mVirtualKeyboard->SetDirty();
}

void PancakeHouseLite::processVirtualKeyboard() {
